...
import com.example.proto.grpc.EventRequest;
import com.example.proto.grpc.EventResponse;
import com.example.proto.grpc.NewdataServiceGrpc;
...
public class NewdataServiceImpl extends NewdataServiceGrpc.NewdataServiceImplBase {
    ...
    public void unaryEvent(EventRequest request, StreamObserver<EventResponse> responseObserver) {
        ...
        EventResponse eventResponse = EventResponse.newBuilder().setResult(request.getSourceId() + request.getEventId()).build();
    ...
    public StreamObserver<EventRequest> clientStreamingEvent(StreamObserver<EventResponse> responseObserver) {
        return new StreamObserver<EventRequest>() {
            ...
            public void onCompleted() {
                //클라이언트로부터 스트림 완료 응답이 오면 1번만 next를 보낼 수 있다. //그래서 위의 onNext에서 같이 onNext하면 안된다.(여러번 응답해버리는 biStream형식이 되어버리기 때문.)
    ...
    public StreamObserver<EventRequest> biStreamingEvent(StreamObserver<EventResponse> responseObserver) {
        return new StreamObserver<EventRequest>() {
            @Override
            public void onNext(EventRequest value) { //클라이언트로부터 데이터가 올 때마다 onNext가 호출된다. //1개의 데이터가 올 때마다 3개의 응답을 던져줄 예정이다.
